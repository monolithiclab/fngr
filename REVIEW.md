...
| -- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F2 | low      | Many exported symbols in `internal/event` and `internal/render` lack godoc comments. Updated list below.                                                         | `event.go`: `Add`, `AddMany`, `AddInput`, `Get`, `Update`, `Delete`, `Reparent`, `AddTags`, `RemoveTags`, `HasChildren`, `List`, `ListSeq`, `GetSubtree`, `ListMeta`, `CountMeta`, `UpdateMeta`, `DeleteMeta`, `Event`, `MetaCount`, `ListOpts`, `ListMetaOpts`, `ErrNotFound`, `ErrCycle`. `meta.go`: `CollectMeta`, `MetaKeyAuthor`/`MetaKeyPeople`/`MetaKeyTag`. `store.go`: `Store`, `NewStore`. `render.go`: `Tree`, `Flat`, `JSON`, `CSV`, `Event`, `FlatStream`, `CSVStream`, `JSONStream`. `markdown.go`: doc comments present on `Markdown`/`MarkdownStream` ✓. `parse.go`: `MetaNameRe` lacks godoc.                                                                                | One-line godoc per symbol — single sentence covering "what does this do" is enough. Improves IDE hover and `go doc` output. Busywork; ride along when nearby files get touched. Bring `MetaNameRe` along the next time `parse.go` is edited.            |
| F6 | medium   | `fngr --version` prints the build's `-X main.version` value, which is currently a commit SHA when no tag exists. Pre-1.0 users see opaque hex.                  | `Makefile` build target wires `-ldflags '-X main.version=<git rev-parse HEAD>'` (or similar) when no tag is reachable.                                                                                                  | Fall back to `git describe --tags --always --dirty` so unreleased builds show `v0.1.0-3-ge4ba501` (clear lineage) instead of bare SHA. One-line Makefile change.                                                                                            |
...
| B10| low      | `parse.MetaArg` returns `KeyValue`'s error bare without the calling context.                                                                                     | `internal/parse/parse.go:90` — `if err != nil { return Meta{}, err }`. Compare with `parse.go:107` (FlagMeta) which wraps with `--meta` context.                                                                          | Wrap: `return Meta{}, fmt.Errorf("parse meta arg %q: %w", s, err)`. Aligns with `FlagMeta`'s pattern.                                                                                                                                                       |
| P5 | low      | `pagerCommand()` re-reads `$PAGER` and re-tokenises on every `withPager` call.                                                                                   | `cmd/fngr/pager.go:69-74`                                                                                                                                                                                              | Cache via `var pagerParts = sync.OnceValue(pagerCommand)`. Sub-microsecond per `fngr list`, but a one-line readability improvement.                                                                                                                       |
| S4 | low      | Migrations are idempotent in practice today (only two), but the pattern relies on the per-migration `tx` rolling back on failure. A future migration that touches multiple statements without `IF NOT EXISTS` could leave the DB half-bumped. | `internal/db/migrate.go::applyMigration`                                                                                                                                                                              | Future-proofing only. Add a doc comment to `applyMigration` reminding writers to use `IF NOT EXISTS` / `IF EXISTS` clauses, and to keep each migration's statements idempotent. No code change today.                                                       |
